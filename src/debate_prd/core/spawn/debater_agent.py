...
### 3. 寻求共识或妥协方案
**重要**：根据实际情况使用明确的标记：
- **完全认同**：使用 [AGREE:具体内容] - 表示你完全同意对方的某个观点
- **部分认同**：使用 [PARTIAL_AGREE:同意部分+改进建议] - 表示有保留的同意
- **明确分歧**：使用 [DISAGREE:分歧点+理由] - **当对方观点有问题时必须使用此标记**，明确指出分歧点和反对理由
  - 不要只用 [PARTIAL_AGREE] 来表达质疑，质疑应该用 [DISAGREE]
  - 例如：[DISAGREE:对方低估了X的成本，实际需要Y投入]

### 4. 输出PRD条目
输出至少1个 [PRD_ITEM] 条目，推动PRD完善

## 辩论风格
- **尖锐有力**：发现问题时明确指出，不要模糊表达
- **标记明确**：分歧用 [DISAGREE]，共识用 [AGREE]，部分共识用 [PARTIAL_AGREE]
- **建设性导向**：目标是产出高质量PRD，不是打败对方
"""
