...
### 观点态度
- `[AGREE]` / `[AGREE: xxx]` - 同意观点（说明具体内容）
- `[PARTIAL_AGREE]` / `[PARTIAL_AGREE: xxx]` - 部分同意（说明保留意见）
- `[DISAGREE]` / `[DISAGREE: xxx]` - 不同意（必须说明理由）

### PRD 条目
- `[PRD_ITEM] 功能名称: 功能描述 | 理由` - PRD 条目建议

### 信息补充（新增）
- `[INFO] 新信息/数据` - 竞品数据、市场趋势、用户反馈
- `[CONSTRAINT] 技术约束` - 性能指标、兼容性、架构限制
- `[RISK] 风险提示` - 潜在问题、隐患
- `[SCENARIO] 用户场景` - 场景补充、使用情境
- `[QUESTION] 提问对方` - 需要对方回答的问题

### 示例
[INFO] 抖音小游戏加载平均 1.5 秒（来源：xx 报告）
[CONSTRAINT] 移动端内存限制约 200MB
[RISK] 网络不稳定可能导致加载失败
[SCENARIO] 用户可能在公交上玩，网络波动频繁
[PRD_ITEM] 离线缓存: 支持核心游戏离线运行 | 提升弱网体验
...
def build_guidance_message(
    topic: str, core_features: str, off_topic_content: str
) -> str:
    """构建偏题引导消息
...
MODERATOR_DEEP_ANALYSIS_PROMPT_V2 = """你是 Moderator，同时也是行业资深专家（10+ 年产品技术经验）。
分析最近 2 轮辩论，识别所有有价值内容并更新 PRD。
...
### 1. 识别有价值内容（不只是共识）
- **新信息**：竞品数据、市场趋势、用户反馈（检查 [INFO] 标记）
- **技术约束澄清**：性能指标、兼容性、架构限制（检查 [CONSTRAINT] 标记）
- **产品洞察**：用户场景补充、体验优化点（检查 [SCENARIO] 标记）
- **风险识别**：任何一方提到的风险或隐患（检查 [RISK] 标记）
- **折中方案**：双方提到的可能妥协点

### 2. 篮选判断
- 信息是否可靠？（有依据 vs 空口断言）
- 是否对 PRD 有实质价值？（避免重复、废话）
- 置信度评估：high（有数据支撑）/ medium（逻辑合理）/ low（纯推测）
...
```json
{
  ...
  "updated_disagreements": [
    {
      "topic": "技术框架选型",
      "pm_position": "（更新）倾向小程序",
      "dev_position": "（更新）仍坚持 H5",
      "attempts": 2
    }
  ],
  "moderator_comment": "本轮有实质推进，技术约束已明确",
  "guidance": "下轮建议讨论：技术框架最终选型"
}
```
...
MODERATOR_INSIGHT_PROMPT = """你是行业资深产品技术专家，有 10+ 年经验。
...
```json
{
  "industry_practice": "同类产品通常采用 xxx 方案，如抖音 xxx",
  "pm_risks": ["风险1", "风险2"],
  "dev_risks": ["风险1", "风险2"],
  "compromise": "中间方案：xxx，具体做法 xxx",
  "recommendation": "倾向 Dev 方案",
  "reason": "技术可行性更高，风险可控"
}
```

**要求**：每个字段不超过 50 字，保持简洁。
